...
//
// @param options {Object}
// @param options.shop {String} The target shop's name, like "my-store" in "my-store.my-shopify.com"
// @params options.keyset {String} Name of keyset where `secret` is stored.
// @params options.api_key {String} Your App's API Key
// @params [options.scopes] {String} Desired API scopes.
// @params [options.embedded_app_sdk] {Boolean} Whether to authenticate as an Embedded App SDK (default is false).
...
Shopify.PublicAppOAuthAuthenticator = function PublicAppOAuthAuthenticator(options) {
    ...
    if (options.secret) {
        throw new Error("Do not provide your shared secret on the client.");
    }

    check(options, Object);
    check(options.shop, Shopify._IsNonEmptyString);
    check(options.keyset, Shopify._IsNonEmptyString);
    check(options.api_key, Shopify._IsNonEmptyString);

    this.config = { };
    this.config.type = "public";
    this.config.shop = options.shop;
    this.config.keyset = options.keyset;
    this.config.api_key = options.api_key;
    this.config.scopes = options.scopes || Shopify.DefaultScopes;
    this.config.embedded_app_sdk = options.embedded_app_sdk || false;
    this.config.post_auth_uri = options.post_auth_uri || "close";
    this.config.onAuth = options.onAuth || function() { };

    // Allow user to explicitly set scopes to "all" as seen in documentation.
    if (options.scopes === "all") {
        this.config.scopes = Shopify.DefaultScopes;
    }
    ...
    //
    // See https://docs.shopify.com/api/authentication/oauth#asking-for-permission
    //
    // We send this redirect_id as the nonce in the state parameter, and when
    // shopify redirects to the redirect_uri we give it (which defaults to
    // "/__shopify-auth"), it gives back the same nonce in the state parameter.
    ...
    if (options.redirect_uri) {
        ...
        // or directly.
        Meteor.call("froatsnook:shopify/HandleAuthRedirect", this.config, function(err, access_token) {
            if (err) {
...
// permanent access_token.  This can be done by calling
